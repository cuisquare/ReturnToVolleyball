...
col_size_large_portrait <- 13
col_size_large_landscape <- 24
if (risk_landscape) {
  col_size_large <- col_size_large_landscape
} else {
  col_size_large <- col_size_large_portrait
}
...
Hazards <- Hazards %>% add_new_hazard(
  ...
  Severity_Before = 4,
  Controls = c(
    "Carry out conservative analysis on likely exposure duration (see Virus Exposure Analysis) DONE",
    "Analysis to factor in specifics of activity such as the court size, number of players per team with safety factor added DONE",
    "Reduce the total duration of play corresponding to an accept risk of virus exposure from accumulated proximity DONE",
...
Hazards %>%
  ...
  column_spec(1, width = get_col_size_string(col_size_large),border_left=T) %>%
  ...
  column_spec(6, width = get_col_size_string(col_size_large))%>%
