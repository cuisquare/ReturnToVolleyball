...
Based on this it is possible to calculate the maximum advisable number of sets per player depending on the chosen acceptable virus exposure per session. As a default in this risk assessment, the acceptable total exposure per session was considered to be **`r max_exposure` minutes**. This compare to the value of *5 minutes* for the VE study and *15 minutes* for the Return to Sport maximum for a Medium risk.   

The rationale for that choice is that we want to remain under the value mandated by government for a medium risk (as this is a requirement of return to sport). However we do not match that actual value because it allows for a safety margin in possible close contact increase owing to the scrappier nature of park play. 

This is informed from attendance of the Webinar where it was explained that the games chosen for evaluation were national league finals, hence a very high level focused game. 

Obviously this choice constitute an educated guess, and it should be validated by observation of typical park play which is an exercise that has not been carried out so far.
