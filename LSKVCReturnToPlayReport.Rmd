...
## Prior to Activity:
* Let us know you plan to attend the activity. This can be made by replying to the event created on Facebook. Numbers are capped at 26, if you have not let us know you are coming, you might not be able to join the activity on the day if numbers are reached.
* Before you attend you need to opt-in to, confirming that you read and understood the risk assessment and agree with the residual risk involved in the activity. Please get in touch with any questions you might have.
...
* Sets will be capped at **15 points** or **20 minutes** with golden point whichever happens first.
* Each player can play a maximum of **`r round(get_max_nb_sets_per_player(8,6,20,1.5,10))` sets** if we play 6v6, **`r round(get_max_nb_sets_per_player(8,5,20,1.5,10))` sets** total if we play 5v5 and **`r round(get_max_nb_sets_per_player(8,4,20,1.5,10))` sets** total if we play 4v4.
...
The duration of set is not fixed by duration but based on number of points. Namely all calculations use the figure 20 minutes for a set, which should be achieved by setting the max number of points to 15 points. This should be monitored on the day and time be controlled if necessary.
